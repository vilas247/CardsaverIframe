				...
				if($valid){
					...
					if(isset($_REQUEST['is_enable']) && $_REQUEST['is_enable'] == "Enable"){
						...
						if (isset($result_s[0])) {
							...
							if($res == "1"){
								$usql = "update cardsaver_token_validation set is_enable=1 where email_id='".$email_id."'";
								//echo $usql;exit;
...
function createCustomPage($email_id,$store_hash,$acess_token){
	...
	$request = array(
			  ...
			  "is_visible"=> false,
			  "mobile_body"=> "",
			  "name"=> "Cardsaver Iframe Custom Order Confirmation",
			  "parent_id"=> 0,
			  "search_keywords"=> "",
			  "sort_order"=> 0,
			  "type"=> "raw",
			  "url"=> "/csiframe-custom-order-confirmation"
			);
...
function createScripts($sellerdb,$acess_token,$store_hash,$email_id){
	...
	foreach($url as $k=>$v){
		...
		$request = '{
		  "name": "CardSaverIframeApp",
		  "description": "CardsaverIframe payment files",
		...
		if(!empty($res)){
			...
			if(isset($response['data']['uuid'])){
				$sql = 'insert into cardsaver_scripts(script_email_id,script_filename,script_code,status,api_response) values("'.$email_id.'","'.basename($v).'","'.$response['data']['uuid'].'","1","'.addslashes($res).'")';
				//echo $sql;exit;
				$conn->exec($sql);
				$rStatus++;
			}
...
$filecontent .= 'function checkCsiframePayBtnVisibility() {
	...
	var key = $("body #247csiframekey").val();
	$.ajax({
		type: "GET",
		dataType: "json",
