...
$logger = new Logger('CARDSAVER Success Order');
$logger->pushHandler(new StreamHandler('var/logs/CARDSAVER_success.txt', Logger::INFO));
$logger->info("response from cardsaver: ".json_encode($_REQUEST));
			...
			if(isset($result[0])) {
				...
				if(!empty($res)){
					...
					if(isset($res['secure_url'])){
						//header("Location:".$res['secure_url']."/checkout?csiframeinv=".base64_encode(json_encode($invoice_id)));die();
						$url = $res['secure_url']."/checkout?csiframeinv=".base64_encode(json_encode($invoice_id));
		...
		if(isset($res['secure_url'])){
			...
			if(isset($invoice_result[0])) {
				...
				if($bg_customer_id > 0){
					...
					$url = $res['secure_url'].'/csiframe-custom-order-confirmation?authKey='.$invoice_id;
					echo '<script>window.parent.location.href="'.$url.'";</script>';
				}	
			}else{
...
function createBGOrder($invoiceId){
	...
	if (isset($result_order_payment[0])) {
		...
		if (isset($result[0])) {
			...
			$createOrder['external_source'] = "247 CARDSAVER";
