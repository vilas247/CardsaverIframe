	...
	if(!empty($email_id)){
		...
		if ($result[0]) {
			...
			if(!empty($result['merchant_id']) && !empty($result['cardstream_signature']) && !empty($result['acess_token']) && !empty($result['store_hash'])){
				...
				if($res == "1"){
					$usql = "update cardsaver_token_validation set is_enable=? where email_id=?";
					//echo $usql;exit;
	...
	foreach($url as $k=>$v){
		...
		if(!empty($res)){
			...
			if(isset($response['data']['uuid'])){
				$sql = 'insert into cardsaver_scripts(script_email_id,script_filename,script_code,status,api_response) values(?,?,?,?,?)';
				//echo $sql;exit;
