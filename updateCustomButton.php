...
if(isset($_REQUEST['container_id'])){
	...
	if(!empty($email_id)){
		...
		if (count($result) > 0) {
			$stmt_c = $conn->prepare("select * from custom_cardsaverpay_button where email_id='".$email_id."'");
			$stmt_c->execute();
			...
			if (count($result_c) > 0) {
				$usql = 'update custom_cardsaverpay_button set container_id="'.$_REQUEST['container_id'].'",css_prop="'.$_REQUEST['css_prop'].'",html_code="'.htmlentities($_REQUEST['html_code']).'",is_enabled="'.$enable.'" where email_id="'.$email_id.'"';
				// execute the query
				$conn->exec($usql);
				$sellerdb = $result[0]['sellerdb'];
				alterFile($sellerdb,$email_id);
			}else{
				$isql = 'insert into custom_cardsaverpay_button(email_id,container_id,css_prop,is_enabled,html_code) values("'.$email_id.'","'.$_REQUEST['container_id'].'","'.$_REQUEST['css_prop'].'","'.$enable.'","'.htmlentities($_REQUEST['html_code']).'")';
...
function alterFile($sellerdb,$email_id){
	...
	if(!empty($sellerdb)){
		...
		$filecontent .= '$("head").append("<link rel=\"stylesheet\" type=\"text/css\" href=\"'.BASE_URL.'css/csiframe-hosted-fields.css\" />");var csiframecsiframehformInitialized = false;';
