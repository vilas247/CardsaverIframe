...
if(isset($_REQUEST['email_id'])){
	...
	if(!empty($email_id)){
		...
		if (isset($result[0])) {
			...
			$usql = "update cardsaver_token_validation set password=? where email_id=?";
...
function sendMail($email,$password){
	if(!empty($email)){
		...
		$mail->setFrom('notifications@247cloudhub.co.uk', 'Card Saver');     //Set who the message is to be sent from
		$mail->addAddress($email);  // Add a recipient
		//$mail->AddCC('krishnamaneni.villu@gmail.com');
		//$mail->addAddress('krishnamaneni.villu@gmail.com');  // Add a recipient
		$mail->isHTML(true);                                  // Set email format to HTML
		$mail->Subject = 'Password Reset(Card Saver)';
		$mail->msgHTML('<html>
			<body>
